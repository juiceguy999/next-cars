...
const CarAddForm = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (fields.images.length + files.length > 5) {
      toast.info('Upload up to 5 photos!')
      return;
    }
    ...
    for (const file of files) {
      ...
      reader.onload = (event) => {
        ...
        if (newImages.length === files.length) {
          setFields((prevFields) => ({
            ...prevFields,
            images: [...prevFields.images, ...newImages]
          }));
        }
      };
  ...
  const handleSubmit = async (e) => {
    ...
    if(fields.images.length === 0) {
      setImagesError('Please upload an image')
      isValid = false;
    } else {
      setImagesError('');
    }
    ...
    const files = fields.images.map((image) => image.file);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to submit the POST request for the car form');
      }

      const result = await response.json();
      router.replace(`/cars/${result.id}`)
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="mb-10">
        <p className="block text-light font-bold mb-4">Choose photos of your car <span className="text-[#ff5700] opacity-90">*</span> <span className="text-silverGray opacity-90 text-xs -translate-y-1/2 inline-block">(max 5 images)</span></p>
        
        <div className="flex flex-wrap gap-4">

          {fields.images.map((image, index) => (
            <div key={index} className={`${index === 0 ? 'w-60 h-60 border border-orange' : 'w-28 h-28'} relative rounded overflow-hidden`}>
              <div className="absolute w-full h-full bg-[rgba(0,0,0,0.3)] z-[1]"></div>
              <Image src={image.url} className="object-cover" fill alt="car"/> 
              <button type="button" onClick={() => handleRemoveImage(index)} className="absolute top-1 right-1 p-1 rounded-md bg-dark cursor-pointer z-[2] border border-borderGray"><CrossIcon className="fill-orange w-4 h-4" /></button>
            </div>
          ))}
          
          { (fields.images.length < 5) && (<label htmlFor="images" className="cursor-pointer w-28 h-28 rounded-lg bg-darkGray flex items-center justify-center">
            <Image src={uploadIcon} alt="upload" width={60} height={60} className="" />
            <input
              type="file"
              id="images"
              accept="image/*"
        ...
        {imagesError && <span className="block text-[#f00] opacity-90 mt-4 text-sm">{imagesError}</span>}
      </div>
